...
expn = glload("../tecounts/genes_ntc_expression.glb")
expn.log(2,.1)
...
cols = shared.get_cols(expn.getConditionNames())
...
pca.scatter(filename="scatter12.png", x=1, y=2, label=True, spot_cols=cols, figsize=size, label_font_size=6, adjust_labels=True)
pca.scatter(filename="scatter23.png", x=2, y=3, label=True, spot_cols=cols, figsize=size, label_font_size=6, adjust_labels=True)
pca.scatter(filename="scatter24.png", x=2, y=4, label=True, spot_cols=cols, figsize=size, label_font_size=6, adjust_labels=True)
pca.scatter(filename="scatter25.png", x=2, y=5, label=True, spot_cols=cols, figsize=size, label_font_size=6, adjust_labels=True)

for interesing_pc in [1,2,3,4,5,6]:
    ...
    [os.remove(f) for f in glob.glob('barh_pc%s_bot/*.png' % (interesing_pc,))]
    [os.remove(f) for f in glob.glob('barh_pc%s_top/*.png' % (interesing_pc,))]
    ...
    lod.heatmap(filename='heat_pc%s_top.png' % interesing_pc, row_label_key='name', bracket=[5, 15],
        row_font_size=6,col_font_size=6, heat_hei=0.008*len(lod), heat_wid=0.6, figsize=[7,13],
        col_colbar=cols)
    lod.boxplot('box_pc%s_top.png' % interesing_pc, size=[10,6])
    lod.unlog(2,.1)
    lod.save('glbs/pc%s_top.glb' % interesing_pc)
    for g in lod:
        lod.barh_single_item(value=g['name'], key="name",
    ...
    lod.heatmap(filename='heat_pc%s_bot.png' % interesing_pc, row_label_key='name', bracket=[5, 15],
        row_font_size=6,col_font_size=6, heat_hei=0.008*len(lod), heat_wid=0.6, figsize=[7,13],
        col_colbar=cols)
    lod.boxplot('box_pc%s_bot.png' % interesing_pc, size=[10,6])
    lod.unlog(2,.1)
