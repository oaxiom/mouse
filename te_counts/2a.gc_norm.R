...
# Load data:
gc = data.frame(fread("../mm10/mm10_ensg_tes_gc_percent_v95.txt"), row.names=1) # The /100.0 is not required
...
for (i in 2:dim(count_table)[2]){
  ...
  plot(density(log10(count_table[,i])), main=title, xlim=c(0, 0.3))
}
dev.off()

pdf("biasplots.pdf", width=15, height=7)
par(mfrow=c(1,2))
plot(density(log10(count_table[,1])), main="Before Norm", ylim=c(0, 0.3))
for (i in 2:dim(count_table)[2]){
...
plot(density(log10(count_table[,1])), main="Post Norm", ylim=c(0, 0.25))
